...
# Things I learned:
# Nonsense is the concept which uses something that you can add which is random you can use to change the program so you cant do a replay attack
# HUFFMAN:
# in compression there is lossy compression, and lossless compression
# jpeg and mp3 are lossy formats
# jpeg takes pixels and tries to find similar regions and averages their values
# mp3 gets rid of frequencies which humans dont hear
# a huffman tree is lossless compression
# in computer science there is a number that is your entropy
# 101010 would be 50% entropy
# would is the chance that the next bit is a 1 or a 0
# something that has structure cannot have 50%
# the lower the entropy is, the more you know what the next bit is
# generally in computer science will have more 1s
# use shannon entropy
# using the frequencies of data, you can conbine it into groups, says huffman
# the bbiggest repition will get the smallest value
# the value is in the use of less storage
# the program will enter the 4 bits into the tree and get the value
# build an algorithm to find the repitions
# FEOF:
# there is blocking and non-blocking modes in input
#
# there are many axis to programming
# use ''' to comment the inline help for functions
# get pep8 and fix up code
...
def stream_recv_message(key, connection):
    ...
    if key is None:
        ...
        message = decode_with_key(new_message, int(key))
    return message
...
class send_commands(cmd.Cmd):
    ...
    def do_stream(self, arg):
        ...
        message = send_message('stream ' + arg, self.key, self.client)
    ...
    def do_transfer(self, arg):
        ...
        message = send_message('transfer ' + arg, self.key, self.client)
