...
import socket, cmd, getpass, os, argparse, time, platform, subprocess, shlex
...
def quick_transfer(totransfer, connection, key, maxsize=1024, ifbigger='more', ifsmaller='less', delafter=1):
    'Use quick_transfer protocol to decide method of sending.'
    if len(totransfer) > maxsize:
        ...
        with open('tmp.txt', 'w') as quick_tmp:
            quick_tmp.write(totransfer)
        print(chunk_transfer('./tmp.txt', connection, key))
        if delafter:
            os.remove('./tmp.txt')
    else:
        message, length_of_msg = send_message(ifsmaller, key, connection)
        message, length_of_msg = send_message(totransfer, key, connection)

def stream_transfer(file_path, chunk_size, connection):
    'Send file using stream protocol.'
    try:
...
def chunk_transfer(file_path, connection, key, chunk_sizes=CHUNK_SIZE):
    'Send file using chunk protocol.'
    with open(file_path, mode='rb') as opened_file:
        try:
            filesize = os.stat(file_path).st_size
            chunk_numbers = []
...
def find_all_files(start_path='//', mode=0):
    'Recursively find all paths of files in start_path.'
    if start_path[-1:] == '/':
...
class recv_commands(cmd.Cmd):
    ...
    def do_stream(self, arg):
        ...
        if os.path.isfile(arg):
            message, length_of_msg = send_message('That file exists and will be streamed over.', self.key, self.server)
            print(stream_transfer(arg, 512, self.server))
            if del_file:
                os.remove(arg)
        else: message, length_of_msg = send_message('That file does not exist.', self.key, self.server)
        return 1
    def do_transfer(self, arg):
        ...
        if os.path.isfile(arg):
            ...
            transfer_success = chunk_transfer(arg, self.server, self.key)
            ...
            if recv_message(self.key, self.server) == 'y':
                ...
                for path in file_list:
                    message, length_of_msg = send_message(arg.split('/')[-1] + '/' + path[len(arg)+1:], self.key, self.server)#the string chopping process removes the path of the this computer which is not being replicated on the clientside
        ...
        while True:
            ...
            if 'cd' == command[:2]:
                ...
                try:
                    ...
                    print(run_command[:101])
                    quick_transfer(run_command, self.server, self.key, maxsize=100)
