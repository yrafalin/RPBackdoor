        ...
        with open(file_path, mode='rb') as opened_file:
            ...
            while True:
                ...
                if opened_file.read(1):
                    ...
                    endfile = b'1'
                message = endfile + ('0' * (len(str(chunk_size))-len(str(chunk_size-len(message)))) +
                str(chunk_size-len(message))).encode() + message + ('0' * (chunk_size-len(message))).encode()
                connection.send(message)
                if opened_file.read(1):
                    opened_file.seek(-1, 1)
                else:
...
class recv_commands(cmd.Cmd):
    ...
    def do_stream(self, arg):
        ...
        if os.path.isfile(arg):
            ...
            message, length_of_msg = send_message('That file does not exist.', self.key, self.server)
        return 1
    ...
    def do_transfer(self, arg):
        ...
        if os.path.isfile(arg):
            ...
            print(file_list)
            message, length_of_msg = send_message('Would you like to transfer all ' +
            '{num} files. That is {byte} bytes. '.format(num=len(file_list), byte=size_of_files), self.key, self.server)
            if recv_message(self.key, self.server) == 'y':
                ...
                for path in file_list:
                    ...
                    message, length_of_msg = send_message(arg.split('/')[-1] + '/' + path[len(arg)+1:], self.key, self.server)
